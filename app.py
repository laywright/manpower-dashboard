...
# Page setup
st.set_page_config(page_title="BasiGo Manpower Dashboard", page_icon="🚌", layout="wide")

# Dashboard title and description
st.title("🚌 BasiGo Manpower Dashboard")
st.markdown("""
### 👨‍🏭 Manpower Efficiency and Process Analytics
Visualize manhour trends, identify process bottlenecks, and understand workforce allocation across stations.
""")
...
if uploaded_file is not None:
    ...
    with tab1:
        ...
        for _, row in top5.iterrows():
            st.markdown(f"• {row['Bus']}: {row['Manhours']:.1f} hrs")

        # Station KPI Comparison
    ...
    with tab2:
        ...
        # Station-specific toggle
        if 'Station' in df.columns:
            station_options = df['Station'].dropna().unique().tolist()
            if station_options:
                selected_station = st.selectbox("🎯 Select Station for Breakdown", station_options, key='station_avg')
                ...
                fig_station = px.bar(station_avg_df, x='Process', y='Avg_Time_Per_Process',
                                     title=f'Avg Time per Process in {selected_station} Station',
                                     labels={'Avg_Time_Per_Process': 'Avg Time (hrs)'},
                                     color_discrete_sequence=['green'], text='Avg_Time_Per_Process')
